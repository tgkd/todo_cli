...
export default class CalendarHeader extends Component {
  ...
  render() {
    ...
    const dayNamesRow = dayNames.map((day, id) => {
      return <td className='calendar__dayname' key={id}>{day}</td>;
    });
    ...
    return (
      ...
      <tr className='calendar__month-name'>
        <td className='calendar__nav-cell'>
        ...
        <td className='calendar__nav-cell' colSpan='5'>
          <span className='calendar__selected-date'>
            { moment().locale('ru').month(month).format('MMMM') } { year }
          </span>
        </td>
        <td className='calendar__nav-cell'>
